...
const fetchCategories = () =>
  fetch(CAT_URL).then((r) => r.json()).then((d) => d.categories || []).catch(() => []);

// ── Product modal with dynamic category hierarchy ─────────────────
const ProductModal = ({ product, categories, onSave, onClose }) => {
  const blank = {
    name:"", category:"", primaryCategory:"", subCategory:"", subCategoryItem:"",
    brand:"", price:"", originalPrice:"", gender:"Unisex", badge:"", description:"",
    material:"", care:"", sizes:"", colors:"", image:"", sku:"",
  };
  ...
  // ── derive column options for selected primary category ──────────
  const selectedCat = categories.find((c) => c.name === form.category || c.name === form.primaryCategory);
  const columnOptions = selectedCat?.columns || [];

  // ── derive sub-category item options for selected column ─────────
  const selectedCol = columnOptions.find((col) => col.title === form.subCategory);
  const subItemOptions = selectedCol?.subCategories || [];

  // When primary category changes → reset sub-fields
  const handleCatChange = (val) => {
    setForm((f) => ({ ...f, category:val, primaryCategory:val, subCategory:"", subCategoryItem:"" }));
  };
  ...
  return (
    <div className="fixed inset-0 z-[200] flex items-center justify-center">
      <div className="absolute inset-0 bg-black/60 backdrop-blur-sm" onClick={onClose} />
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[92vh] overflow-y-auto mx-4 z-10">
        <div className="flex items-center justify-between px-6 py-5 border-b border-gray-100 sticky top-0 bg-white z-10">
          <div>
            <h2 className="font-semibold text-gray-900 text-lg">{product ? "Edit Product" : "Add New Product"}</h2>
            <p className="text-xs text-gray-400 mt-0.5">Fields marked * required</p>
        ...
        <div className="p-6 space-y-5">
          ...
          <div className="bg-violet-50 rounded-2xl p-4 space-y-3 border border-violet-100">
            <p className="text-xs font-bold text-violet-700 uppercase tracking-widest mb-1">Category Hierarchy</p>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              {/* Primary category */}
              <div>
                <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5">Primary Category *</label>
                <select value={form.category||""} onChange={(e) => handleCatChange(e.target.value)}
                  className="w-full px-4 py-2.5 rounded-xl border border-gray-200 text-sm outline-none focus:border-violet-500 bg-white">
                  <option value="">— Select —</option>
                  {categories.map((c) => <option key={c._id} value={c.name}>{c.icon} {c.label||c.name}</option>)}
                </select>
              </div>

              {/* Sub-category (column) */}
              <div>
                <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5">Column / Group</label>
                <select value={form.subCategory||""} onChange={(e) => setForm((f)=>({...f,subCategory:e.target.value,subCategoryItem:""}))}
                  disabled={!columnOptions.length}
                  className="w-full px-4 py-2.5 rounded-xl border border-gray-200 text-sm outline-none focus:border-violet-500 bg-white disabled:opacity-50">
                  <option value="">— Select —</option>
                  {columnOptions.map((col) => <option key={col._id} value={col.title}>{col.title}</option>)}
                </select>
              </div>

              {/* Sub-category item (leaf) */}
              <div>
                <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5">Sub-Category Item</label>
                <select value={form.subCategoryItem||""} onChange={(e) => set("subCategoryItem", e.target.value)}
                  disabled={!subItemOptions.length}
                  className="w-full px-4 py-2.5 rounded-xl border border-gray-200 text-sm outline-none focus:border-violet-500 bg-white disabled:opacity-50">
                  <option value="">— Select —</option>
                  {subItemOptions.map((sub) => <option key={sub._id} value={sub.name}>{sub.name}</option>)}
                </select>
              </div>
            </div>
            ...
            {form.category && (
              <p className="text-xs text-violet-500 font-medium">
                📂 {[form.category, form.subCategory, form.subCategoryItem].filter(Boolean).join(" › ")}
              </p>
            )}
          </div>
          ...
          <div className="grid grid-cols-3 gap-4">
            {field("Selling Price ₹ *", "price",         { type:"number", placeholder:"4999" })}
            {field("MRP ₹",             "originalPrice",  { type:"number", placeholder:"7999" })}
            {field("Badge",             "badge",          { placeholder:"New / Hot / Sale" })}
          </div>

          {/* Row 4 — variants */}
          <div className="grid grid-cols-3 gap-4">
            {field("Sizes (comma-sep)", "sizes",  { placeholder:"S, M, L, XL" })}
            {field("Colors (comma-sep)","colors", { placeholder:"Black, White" })}
            {field("Gender",            "gender", { select:true, options:["Men","Women","Unisex","Boys","Girls"] })}
          </div>
...
const AdminProducts = ({ products, setProducts, loading, errors, onRetry, notify, searchQuery }) => {
  ...
  // Load categories for the product modal dropdowns
  useEffect(() => { fetchCategories().then(setCategories); }, []);

  const filtered = searchQuery
    ? products.filter((p) => [p.name, p.category, p.brand, p.subCategory, p.subCategoryItem].some((f) => (f||"").toLowerCase().includes(searchQuery.toLowerCase())))
    : products;
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between">
        <p className="text-white/40 text-sm">{filtered.length} products</p>
        <div className="flex gap-2">
          <button onClick={onRetry} className="text-xs text-violet-400 hover:text-violet-300 px-3 py-2 rounded-xl border border-violet-500/20 hover:border-violet-500/40 transition-all">↻ Refresh</button>
          <button onClick={() => setProductModal("new")}
            className="flex items-center gap-2 px-5 py-2.5 rounded-xl bg-gradient-to-r from-violet-600 to-purple-600 text-white text-sm font-semibold hover:shadow-lg hover:shadow-violet-900/50 transition-all">
            + Add Product
          </button>
              ...
              <div key={id} className="bg-[#130d24] border border-white/5 rounded-2xl overflow-hidden hover:border-violet-500/20 transition-all">
                <div className="flex gap-4 p-4">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-white/30 text-xs">{p.brand} · {p.category}</p>
                    {/* Category breadcrumb */}
                    {(p.subCategory || p.subCategoryItem) && (
                      <p className="text-violet-400/60 text-[10px] mt-0.5 truncate">
                        {[p.category, p.subCategory, p.subCategoryItem].filter(Boolean).join(" › ")}
                      </p>
                    )}
                    <div className="flex items-center gap-2 mt-2">
                      <span className="text-white font-bold text-sm">₹{(p.price||0).toLocaleString()}</span>
