...
const ProductDetails = () => {
  ...
  useEffect(() => {
    if (!product || activeTab !== "reviews") return;
    setReviewsLoading(true);
    fetch(`${API_BASE}/products/${product._id}/reviews`)
      ...
      .catch(() => setReviews([
        ...
        { _id:2, name:"Rahul M.",  avatar:"R", rating:4, date:"8 Feb 2025",  comment:"Great product, very well made. Worth every rupee." },
        { _id:3, name:"Aisha K.",  avatar:"A", rating:5, date:"3 Mar 2025",  comment:"Exactly as described. Premium material, accurate sizing." },
        { _id:4, name:"Vikram T.", avatar:"V", rating:3, date:"20 Mar 2025", comment:"Looks even better in person than in the photos." },
      ]))
  ...
  // ── Inline add-to-cart (used from the detail page directly) ─────
  // Used by the desktop CTA and the mobile sticky bar.
  // If size/colour haven't been chosen yet, opens the modal instead of
  // calling addToCart directly so the user is properly prompted.
  const handleAddToCart = async () => {
    const needsSize  = product.sizes?.length  > 0 && !selectedSize;
    const needsColor = product.colors?.length > 0 && !selectedColor;
    if (needsSize || needsColor) {
      // Open modal in cart mode — it will handle validation + addToCart
      setModalMode("cart");
      return;
    }
    await addToCart(product, selectedSize, selectedColor, quantity);
      ...
      <div className="min-h-screen" style={{ background: "#f9f9fb" }}>
        ...
        <div className="max-w-screen-xl mx-auto px-4 py-8">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
            ...
            <div className="space-y-4">
              {[4, 8, 6, 12, 10].map((h, i) => (
                <Skeleton key={i} className={`h-${h} ${i < 2 ? "w-3/4" : "w-full"}`} />
              ))}
            </div>
          </div>
        </div>
  ...
  return (
    <div className="min-h-screen pb-28 lg:pb-0" style={{ background: "#f9f9fb", fontFamily:"'DM Sans',system-ui,sans-serif" }}>
      <style>{`
        @import url('https://fonts.googleapis.com/css2?family=Cormorant+Garamond:ital,wght@0,300;0,400;0,500;0,600;1,300&family=DM+Sans:opsz,wght@9..40,300;9..40,400;9..40,500;9..40,600&display=swap');
        .pd-serif{font-family:'Cormorant Garamond',Georgia,serif!important;}
        .pd-topbar{position:sticky;top:0;z-index:50;background:linear-gradient(135deg,#0f0820,#1e0a3c 60%,#160630);border-bottom:1px solid rgba(255,255,255,0.06);box-shadow:0 4px 30px rgba(15,5,30,0.5);}
        .pd-size{padding:9px 18px;border-radius:12px;border:1.5px solid #e9d5ff;font-size:13px;font-weight:600;cursor:pointer;transition:all 0.18s;background:white;color:#374151;}
        .pd-size:hover{border-color:#a855f7;color:#7c3aed;}
        .pd-size.active{background:linear-gradient(135deg,#1e0a3c,#7c3aed);color:white;border-color:transparent;box-shadow:0 4px 14px rgba(124,58,237,0.32);}
        .pd-tab{padding:14px 28px;font-size:13px;font-weight:600;border-bottom:2px solid transparent;cursor:pointer;transition:all 0.2s;color:#6b7280;letter-spacing:0.03em;}
        .pd-tab.active{border-color:#7c3aed;color:#7c3aed;background:rgba(124,58,237,0.04);}
        .pd-tab:hover:not(.active){color:#7c3aed;}
        .pd-wish{width:44px;height:44px;border-radius:50%;background:white;border:1.5px solid #f3f4f6;display:flex;align-items:center;justify-content:center;font-size:20px;cursor:pointer;transition:all 0.2s;box-shadow:0 2px 8px rgba(0,0,0,0.08);}
        .pd-wish:hover{transform:scale(1.12);border-color:#fca5a5;}
        .pd-wish.active{background:#fff1f2;border-color:#fca5a5;}
        .pd-thumb{width:76px;height:76px;border-radius:14px;overflow:hidden;border:2px solid transparent;cursor:pointer;transition:all 0.18s;flex-shrink:0;}
        ...
        .pd-thumb:not(.active):hover{border-color:#ddd6fe;}
        .pd-atc{flex:1;padding:16px;border-radius:20px;font-size:14px;font-weight:700;border:none;cursor:pointer;transition:all 0.25s;letter-spacing:0.04em;}
        .pd-atc-main{background:linear-gradient(135deg,#1e0a3c,#7c3aed 60%,#a855f7);color:white;}
        .pd-atc-main:hover{transform:translateY(-2px);box-shadow:0 8px 25px rgba(124,58,237,0.4);}
        .pd-atc-main.success{background:linear-gradient(135deg,#10b981,#059669);}
        .pd-delivery{background:linear-gradient(135deg,#faf5ff,#f5f3ff);border:1px solid #e9d5ff;border-radius:20px;padding:18px;}
        .pd-trust{background:white;border:1px solid #f3f4f6;border-radius:16px;padding:14px;display:flex;flex-direction:column;align-items:center;gap:6px;text-align:center;}
      ...
      <div className="pd-topbar">
        <div className="max-w-screen-xl mx-auto px-4 sm:px-6 h-14 flex items-center gap-4">
          <button onClick={() => navigate(-1)} className="flex items-center gap-2 text-white/60 hover:text-white text-xs font-semibold tracking-wider uppercase px-3 py-1.5 rounded-xl hover:bg-white/8 transition-all">← Back</button>
          <span className="pd-serif text-sm text-purple-200 tracking-wide flex-1 truncate">{product.name}</span>
          <div className="flex items-center gap-2">
            <button onClick={() => toggleWishlist(product)} className={`pd-wish ${wishlisted ? "active" : ""}`} style={{ color: wishlisted ? "#ef4444" : "rgba(107,114,128,0.7)" }}>
              {wishlisted ? "♥" : "♡"}
            </button>
            <button onClick={() => navigate("/cart")} className="relative w-10 h-10 rounded-xl bg-white/8 flex items-center justify-center text-white/60 hover:text-white hover:bg-white/15 transition-all text-lg">
              🛒
      ...
      <div className="pd-crumb">
        <span className="link" onClick={() => navigate("/dashboard")}>Home</span>
        <span>›</span>
        <span className="link" onClick={() => navigate(`/category/${encodeURIComponent(product.category)}`)}>{product.category}</span>
        <span>›</span>
        <span className="text-gray-700 font-semibold truncate">{product.name}</span>
      </div>
      ...
      <div className="max-w-screen-xl mx-auto px-4 py-6 sm:py-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 mb-14">
          ...
          <div className="flex gap-4">
            {images.length > 1 && (
              <div className="flex flex-col gap-3 flex-shrink-0">
                {images.map((img, i) => (
                  <button key={i} onClick={() => setSelectedImage(i)} className={`pd-thumb ${selectedImage === i ? "active" : ""}`}>
                    <img src={img} alt={`View ${i + 1}`} className="w-full h-full object-cover" onError={(e) => { e.target.src = "https://placehold.co/76x76/f3f4f6/9ca3af?text=L"; }} />
                  </button>
                ))}
              </div>
            )}
            <div className="flex-1 relative">
              {product.badge && <span className="pd-badge">{product.badge}</span>}
              {!product.badge && discount >= 30 && <span className="pd-sale-badge">{discount}% OFF</span>}
              <button onClick={() => toggleWishlist(product)} className={`absolute top-4 right-4 z-10 pd-wish ${wishlisted ? "active" : ""}`} style={{ color: wishlisted ? "#ef4444" : "#d1d5db" }}>
                {wishlisted ? "♥" : "♡"}
              </button>
              <div className="rounded-2xl overflow-hidden aspect-square" style={{ background:"linear-gradient(135deg,#faf5ff,#f5f3ff)" }}>
                <img src={images[selectedImage] || images[0]} alt={product.name} className="w-full h-full object-cover transition-all duration-400" onError={(e) => { e.target.src = "https://placehold.co/600x600/f3f4f6/9ca3af?text=Lyra"; }} />
              </div>
              {images.length > 1 && (
                <>
                  <button className="absolute left-3 top-1/2 -translate-y-1/2 w-9 h-9 bg-white/85 rounded-full shadow flex items-center justify-center text-gray-600 hover:bg-white transition-all text-lg" onClick={() => setSelectedImage((i) => (i - 1 + images.length) % images.length)}>‹</button>
                  <button className="absolute right-3 top-1/2 -translate-y-1/2 w-9 h-9 bg-white/85 rounded-full shadow flex items-center justify-center text-gray-600 hover:bg-white transition-all text-lg" onClick={() => setSelectedImage((i) => (i + 1) % images.length)}>›</button>
                </>
              )}
            </div>
          </div>
          ...
          <div className="flex flex-col gap-5">
            ...
            {product.colors?.length > 0 && (
              <div>
                <p className="text-sm font-semibold text-gray-700 mb-3">Color: <span className="text-violet-600">{selectedColor}</span></p>
                <div className="flex gap-3 flex-wrap">
                  {product.colors.map((c) => (
                    <button key={c} title={c} onClick={() => setSelectedColor(c)}
                      className={`w-10 h-10 rounded-full transition-all hover:scale-110 ${selectedColor === c ? "ring-2 ring-violet-600 ring-offset-2 scale-110" : "ring-1 ring-gray-200"}`}
                      style={{ background: COLOR_MAP[c] || "#ccc", border: c === "White" ? "1.5px solid #e5e7eb" : "none" }} />
                  ))}
                </div>
              </div>
            ...
            {product.sizes?.length > 0 && (
              <div>
                <div className="flex items-center justify-between mb-3">
                  <p className="text-sm font-semibold text-gray-700">Size: <span className="text-violet-600">{selectedSize}</span></p>
                  <button className="text-xs text-violet-500 hover:text-violet-700 font-semibold transition-colors">Size Guide →</button>
                </div>
                <div className="flex gap-2 flex-wrap">
                  {product.sizes.map((s) => (
                    <button key={s} onClick={() => setSelectedSize(s)} className={`pd-size ${selectedSize === s ? "active" : ""}`}>{s}</button>
                  ))}
                </div>
                {!selectedSize && <p className="text-xs text-red-500 mt-2">Please select a size</p>}
              </div>
            ...
            {/* ── CTAs ─────────────────────────────────────────────
                ...
                "Add to Cart" opens the modal in cart mode.
                "Buy Now"     opens the modal in buynow mode — after
                              adding the item the modal navigates to /cart.
            ──────────────────────────────────────────────────────── */}
            <div className="flex gap-3">
              <button
                onClick={() => setModalMode("cart")}
                className="pd-atc pd-atc-main"
              >
                🛒 Add to Cart
              </button>
              <button
            ...
            <div className="pd-delivery">
              ...
              <div className="flex gap-2">
                <input type="text" maxLength={6} value={pincode}
                  ...
                  placeholder="Enter pincode"
                  className="flex-1 px-4 py-2.5 rounded-xl border border-violet-200 text-sm outline-none focus:border-violet-500 bg-white" />
                <button onClick={checkPincode} className="px-5 py-2.5 rounded-xl text-sm font-bold text-white transition-colors" style={{ background:"linear-gradient(135deg,#7c3aed,#a855f7)" }}>Check</button>
              </div>
              {pincodeMsg && <p className={`text-xs mt-2 font-semibold ${pincodeMsg.startsWith("✓") ? "text-emerald-600" : "text-red-500"}`}>{pincodeMsg}</p>}
            </div>
        ...
        <div className="bg-white rounded-2xl border border-violet-50 mb-12 overflow-hidden" style={{ boxShadow:"0 4px 20px rgba(124,58,237,0.06)" }}>
          <div className="flex border-b border-violet-50 overflow-x-auto">
            {["description","details","reviews"].map((tab) => (
              <button key={tab} onClick={() => setActiveTab(tab)} className={`pd-tab whitespace-nowrap ${activeTab === tab ? "active" : ""}`}>
                {tab === "reviews" ? `Reviews (${(product.reviews||0).toLocaleString()})` : tab.charAt(0).toUpperCase() + tab.slice(1)}
              </button>
            ))}
          </div>
          <div className="p-6 sm:p-8">
            {activeTab === "description" && (
              <div className="max-w-2xl">
                <p className="text-gray-600 leading-relaxed text-sm mb-6">{product.description || "No description available."}</p>
                <div className="grid grid-cols-2 gap-4">
                  {[{ label:"Brand", value:product.brand },{ label:"Category", value:product.category },{ label:"Gender", value:product.gender },{ label:"SKU", value:product.sku }].filter((d) => d.value).map((d) => (
                    <div key={d.label} className="flex gap-3">
                      <span className="text-[10px] font-bold text-gray-400 uppercase tracking-wider w-20 flex-shrink-0 pt-0.5">{d.label}</span>
                      <span className="text-sm text-gray-700 font-medium">{d.value}</span>
                    </div>
            ...
            {activeTab === "details" && (
              <div className="max-w-2xl">
                {[{ label:"Material", value:product.material },{ label:"Care", value:product.care },{ label:"Available Sizes", value:product.sizes?.join(", ") },{ label:"Available Colors", value:product.colors?.join(", ") },{ label:"SKU", value:product.sku }].filter((d) => d.value).map((d) => (
                  <div key={d.label} className="flex gap-4 py-3 border-b border-violet-50 last:border-0">
                    <span className="text-[10px] font-bold text-gray-400 uppercase tracking-wider w-36 flex-shrink-0 pt-0.5">{d.label}</span>
                    <span className="text-sm text-gray-700">{d.value}</span>
                  </div>
                ))}
                {!product.material && !product.care && <p className="text-sm text-gray-400">No additional details available.</p>}
              </div>
            )}
            {activeTab === "reviews" && (
              <div>
                ...
                {reviewsLoading ? (
                  ...
                  <div className="space-y-6">
                    {reviews.map((r, idx) => (
                      <div key={r._id || idx} className="review-item flex gap-4" style={{ animationDelay:`${idx*0.08}s` }}>
                        <div className="w-10 h-10 rounded-full flex items-center justify-center font-bold text-sm flex-shrink-0" style={{ background:"linear-gradient(135deg,#ede9fe,#ddd6fe)", color:"#7c3aed" }}>
                          {r.avatar || (r.name||"U").charAt(0).toUpperCase()}
                        </div>
                        <div>
                          <div className="flex items-center gap-3 mb-1 flex-wrap">
        ...
        {related.length > 0 && (
          <div>
            <h2 className="pd-serif text-2xl sm:text-3xl text-gray-900 mb-6">You may also <em className="text-violet-600 not-italic">like</em></h2>
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
              {related.map((p) => {
                const d2 = p.originalPrice ? Math.round(((p.originalPrice-p.price)/p.originalPrice)*100) : 0;
                return (
                  <div key={p._id} onClick={() => { navigate(`/product/${p._id}`); window.scrollTo(0,0); }} className="pd-related group">
                    <div className="overflow-hidden aspect-[3/4]" style={{ background:"#faf5ff" }}>
                      <img src={p.images?.[0]||p.image} alt={p.name} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" onError={(e) => { e.target.src="https://placehold.co/300x400/f3f4f6/9ca3af?text=Lyra"; }} />
                    </div>
                    <div className="p-3">
                      <p className="text-[10px] font-bold text-violet-400 uppercase tracking-wider mb-1">{p.brand}</p>
                      <p className="text-sm font-medium text-gray-900 truncate mb-2">{p.name}</p>
                      <div className="flex items-center gap-2 flex-wrap">
                        <span className="font-bold text-gray-900 text-sm">₹{(p.price||0).toLocaleString()}</span>
                        {p.originalPrice && <span className="text-xs text-gray-400 line-through">₹{p.originalPrice.toLocaleString()}</span>}
                        {d2 > 0 && <span className="text-[10px] font-bold text-emerald-600 bg-emerald-50 px-1.5 py-0.5 rounded">{d2}% off</span>}
                      </div>
                      <button onClick={(e) => { e.stopPropagation(); toggleWishlist(p); }} className="mt-2 text-sm" style={{ color:isWishlisted(p._id)?"#ef4444":"#d1d5db", background:"none", border:"none", cursor:"pointer" }}>
                        {isWishlisted(p._id) ? "♥ Wishlisted" : "♡ Wishlist"}
                      </button>
                    </div>
                  </div>
      ...
      {/* ── MOBILE STICKY BOTTOM BAR ─────────────────────────────────
          On mobile the desktop CTA buttons are hidden (pb-28 + lg:pb-0).
          The sticky bar gives the same three actions:
            - Wishlist toggle
            - Add to Cart  (opens modal in cart mode)
            - Buy Now      (opens modal in buynow mode)
      ──────────────────────────────────────────────────────────────── */}
      <div className="pd-bottom lg:hidden">
        <button
          onClick={() => toggleWishlist(product)}
          className="w-12 h-12 rounded-2xl flex items-center justify-center text-xl flex-shrink-0"
          style={{ background: wishlisted ? "#fff1f2" : "#f9f5ff", border:`1.5px solid ${wishlisted?"#fca5a5":"#e9d5ff"}`, color: wishlisted ? "#ef4444" : "#a78bfa" }}
        >
          {wishlisted ? "♥" : "♡"}
        </button>
        <button
      ...
      {/* ── UNIFIED MODAL
           Renders for both Add to Cart and Buy Now.
           `mode` prop controls the label and post-add behaviour.
           `navigate` is passed so the modal can go to /cart after Buy Now.
      ──────────────────────────────────────────────────────────────── */}
      {modalMode && (
        <AddToCartModal
          product={product}
