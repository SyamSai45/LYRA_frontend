...
const CinematicHero = ({ slide, currentIndex, total, onDotClick, onShopClick }) => {
  ...
  return (
    <div className="relative w-full overflow-hidden lyra-hero">
      ...
      <div className="relative z-10 h-full flex flex-col justify-center px-5 sm:px-12 lg:px-24">
        <p className="text-[10px] sm:text-xs tracking-[0.4em] text-white/40 uppercase mb-3 sm:mb-5 font-light">
          ✦ Lyra Collection
        </p>
        <h1
          className="font-light text-white leading-none mb-4 sm:mb-5 lyra-serif"
        ...
        <p className="text-white/55 text-sm sm:text-base max-w-xs sm:max-w-lg mb-7 sm:mb-9 font-light leading-relaxed">
          {slide.subtitle}
        </p>
        <div className="flex flex-wrap gap-3">
          <button onClick={onShopClick}
            className="px-6 sm:px-10 py-3 sm:py-4 bg-white text-[#1e0a3c] text-xs sm:text-sm font-bold rounded-2xl hover:bg-violet-50 hover:shadow-2xl hover:-translate-y-1 transition-all duration-300">
            {slide.cta} →
          </button>
          <button onClick={onShopClick}
            className="px-5 sm:px-8 py-3 sm:py-4 border-2 border-white/30 text-white text-xs sm:text-sm font-semibold rounded-2xl hover:bg-white/10 hover:border-white/60 transition-all duration-300">
            View Collections
          </button>
        </div>
        <div className="flex gap-2 mt-7 sm:mt-10">
          {Array.from({ length: total }).map((_, i) => (
            <button key={i} onClick={() => onDotClick(i)}
              className={`rounded-full transition-all ${i === currentIndex ? "w-8 h-1.5 bg-white" : "w-4 h-1.5 bg-white/35"}`} />
          ))}
        </div>
      </div>
    </div>
...
const EditorialStrip = ({ onShopClick }) => {
  ...
  return (
    <div
      className="lyra-editorial-strip overflow-hidden relative"
    ...
    >
      ...
      />
      <div className="absolute top-8 left-1/4 w-64 h-64 rounded-full opacity-10 blur-3xl pointer-events-none"
      ...
      <div className="relative z-10 flex flex-col lg:flex-row items-stretch" style={{ minHeight: "400px" }}>
        <div className="flex-1 flex flex-col justify-center px-6 sm:px-12 lg:px-16 py-12 lg:py-16">
          ...
          <h2
            className="lyra-serif text-white font-light leading-[0.95] mb-5"
          ...
          {[
            ...
            <div key={i} onClick={onShopClick}
              ...
              <img src={tile.img} alt={tile.label}
                className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-700" />
              <div className="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent" />
              <span className="absolute bottom-2 left-3 text-white text-[10px] font-bold tracking-widest uppercase opacity-80">
                {tile.label}
              </span>
              <div className="absolute inset-0 border-2 border-transparent group-hover:border-violet-400/40 rounded-xl transition-all duration-300" />
            </div>
  ...
  return (
    <div className="mb-12">
      <div className="flex items-center justify-between mb-5">
        ...
        <button className="text-xs text-violet-600 font-semibold border border-violet-200 hover:border-violet-400 px-4 py-1.5 rounded-full transition-colors">
          View All →
        </button>
      </div>
      <div
        className="relative overflow-hidden bg-white rounded-2xl border border-slate-100 py-5 shadow-sm"
        onMouseEnter={() => { pausedRef.current = true; }}
        onMouseLeave={() => { pausedRef.current = false; }}
      >
        ...
        <div ref={trackRef} className="flex items-center gap-3 px-4"
          ...
          {BRANDS_LOOP.map((b, i) => (
            ...
            >
              <div
                className="w-24 h-12 rounded-xl flex items-center justify-center border border-slate-100 group-hover:border-violet-300 group-hover:shadow-md group-hover:-translate-y-0.5 transition-all overflow-hidden px-3"
...
const ProductCard = ({ product, onAddToCart }) => {
  ...
  return (
    ...
    >
      {product.badge && (
        <span className="absolute top-3 left-3 z-10 bg-gradient-to-r from-violet-600 to-pink-500 text-white text-[10px] font-bold px-2.5 py-1 rounded-full uppercase tracking-wider shadow">
          {product.badge}
        </span>
      )}
      {disc >= 30 && !product.badge && (
      ...
      <div className="p-3 sm:p-4">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {disc > 0 && disc < 30 && (
            <span className="text-[10px] font-semibold text-emerald-600 bg-emerald-50 px-2 py-0.5 rounded-lg">{disc}% off</span>
          )}
        </div>
        <button
        ...
        <div className="p-4">
          ...
          {(categories || []).map((cat) => (
            <button
              ...
              onClick={() => { navigate(`/category/${encodeURIComponent(cat.slug || cat.name)}`); onClose(); }}
              className="w-full flex items-center justify-between px-3 py-3 rounded-xl text-sm font-medium text-gray-700 hover:bg-violet-50 hover:text-violet-700 transition-colors"
            >
              ...
              {cat.badge && (
                <span className="text-[9px] font-bold text-red-500 bg-red-50 px-2 py-0.5 rounded-full">{cat.badge}</span>
              )}
            </button>
          ))}
          <div className="border-t border-slate-100 mt-3 pt-3">
            ...
            <button
              ...
              className="w-full text-left px-3 py-3 rounded-xl text-sm text-red-500 hover:bg-red-50 transition-colors mt-1"
            >
              🚪 Sign Out
            </button>
          </div>
...
// ════════════════════════════════════════════════════════════════
// MEGA NAV — fully fixed for DB data shape
//
// Key fixes applied:
//  1. Uses item.slug as the stable hover/open key (not _id which is an
//     ObjectId and can cause stale-closure issues with setTimeout).
//  2. Filters out columns that have zero valid subCategories so empty
//     columns never appear in the panel.
//  3. Sub-category items are rendered as `sub.name` with a null guard
//     `.filter((s) => s && s.name)` — no JSX syntax error.
//  4. Panel minWidth is computed from *valid* columns only.
//  5. noMenu is coerced with Boolean() so undefined/null/false/"false"
//     are all handled correctly.
//  6. URL is fully encoded: /category/:slug?sub=:subName
//  7. activeRef pattern ensures closeMega never fires after openMega
//     when the user moves between adjacent items quickly.
// ════════════════════════════════════════════════════════════════
const MegaNav = ({ categories, navigate }) => {
  ...
  const openMega = useCallback((key) => {
    // Cancel every pending close timer
  ...
  return (
    <div
      className="lyra-mega-nav relative border-t border-white/[0.06]"
      style={{ background: "rgba(0,0,0,0.18)" }}
    >
      <div className="flex items-stretch justify-center max-w-screen-xl mx-auto overflow-x-auto scrollbar-none">
        {(categories || []).map((item, idx) => {
          // ── stable key: prefer slug, fall back to name ──────────
          const itemKey     = item.slug || item.name;
          const isOpen      = activeMega === itemKey;
          const isNoMenu    = Boolean(item.noMenu);
          const isSale      = item.name === "Sale" || item.slug === "sale";
          const anchorRight = idx >= Math.floor((categories || []).length / 2);

          // ── only columns that have at least one named sub-category ──
          const validCols = (item.columns || []).filter(
            (col) => Array.isArray(col.subCategories) && col.subCategories.some((s) => s && s.name)
          );
          const colCount   = Math.min(validCols.length, 5);
          const panelWidth = colCount <= 1 ? 260 : colCount <= 2 ? 400 : colCount <= 3 ? 540 : colCount <= 4 ? 680 : 820;

          // ── does this item have anything to show in a dropdown? ──
          const hasDropdown = !isNoMenu && colCount > 0;

          return (
            <div
              key={itemKey}
              className="mega-parent flex-shrink-0"
            ...
            >
              ...
              <button
                onClick={() => {
                  if (isNoMenu || !hasDropdown) {
                    navigate(`/category/${encodeURIComponent(item.slug || item.name)}`);
                  } else {
                    setActiveMega((v) => (v === itemKey ? null : itemKey));
                  }
                }}
                className={[
                  "nav-item",
              ...
              >
                ...
                {item.badge && (
                  <span
                    className="ml-2 text-[9px] font-bold px-1.5 py-0.5 rounded-full text-white"
                ...
                {hasDropdown && (
                  <svg
                    className={`ml-1 w-3 h-3 opacity-30 transition-transform duration-200 ${isOpen ? "rotate-180" : ""}`}
              ...
              {hasDropdown && isOpen && (
                ...
                >
                  <div className="flex">
                    {/* ── Columns grid ── */}
                    <div
                      className="flex-1 px-5 py-5 grid"
                      style={{ gridTemplateColumns: `repeat(${colCount}, 1fr)`, columnGap: "14px" }}
                    >
                      {validCols.slice(0, 5).map((col) => (
                        <div key={col._id || col.title} className="py-1">
                          ...
                          <div className="space-y-1.5">
                            {/* FIX: filter ensures only objects with a name render;
                                wrapping in {} makes JSX parser happy */}
                            {col.subCategories
                              .filter((s) => s && s.name)
                              .map((sub) => (
                                <span
                                  ...
                                  onClick={() => {
                                    navigate(
                                      `/category/${encodeURIComponent(item.slug || item.name)}?sub=${encodeURIComponent(sub.name)}`
                                    );
                                    activeRef.current = null;
                                    setActiveMega(null);
                                  }}
                    ...
                    {item.featuredImg ? (
                      <div className="w-40 flex-shrink-0 relative overflow-hidden border-l border-gray-100">
                        <img
                          src={item.featuredImg}
                          alt={item.featuredLabel || item.name}
                          className="w-full h-full object-cover hover:scale-105 transition-transform duration-500"
                          onError={(e) => { e.target.parentElement.style.display = "none"; }}
                        />
                        <div
                          className="absolute inset-0"
                          style={{ background: "linear-gradient(to top,rgba(15,5,30,0.75) 0%,transparent 55%)" }}
                        />
                        <div className="absolute bottom-0 left-0 right-0 p-3">
                          <p className="text-[9px] tracking-widest text-white/50 uppercase mb-1">Featured</p>
                          ...
                          <button
                            ...
                            className="mt-1.5 text-[10px] text-white/70 hover:text-white transition-colors"
                          >
                            Shop Now →
                          </button>
                        </div>
...
const Dashboard = () => {
  ...
  const dealsProducts = products
    .filter((p) => p.originalPrice && ((p.originalPrice - p.price) / p.originalPrice) >= 0.3)
    .slice(0, 10);
  ...
  const showcaseCats = (categories || []).slice(0, 8).map((cat) => ({
    ...
    img:   cat.featuredImg || cat.heroImg || SHOWCASE_IMGS[cat.slug] || "",
  ...
  return (
    <div className="min-h-screen bg-[#f9f9fb]">
      <style>{`
        ...
        .mega-link::after { content:''; position:absolute; bottom:0; left:0; width:0; height:1px; background:#7c3aed; transition:width 0.2s ease; }
        .mega-link:hover::after { width:100%; }
        .nav-item { position:relative; padding:0 13px; height:42px; display:flex; align-items:center; font-size:11px; font-weight:600; letter-spacing:0.07em; text-transform:uppercase; cursor:pointer; transition:color 0.2s,background 0.2s; color:rgba(255,255,255,0.5); white-space:nowrap; }
        .nav-item:hover { color:white; background:rgba(255,255,255,0.05); }
        .nav-item.active { color:white; background:rgba(255,255,255,0.05); }
        .nav-item::after { content:''; position:absolute; bottom:0; left:13px; right:13px; height:2px; background:white; transform:scaleX(0); transition:transform 0.2s ease; border-radius:2px 2px 0 0; }
        .nav-item:hover::after,.nav-item.active::after { transform:scaleX(1); }
        .nav-item.sale-item { color:#f87171; }
        .nav-item.sale-item:hover { color:#fca5a5; }
        .nav-item.sale-item::after { background:#ef4444; }
      ...
      <div className="lyra-body">
        ...
        <nav
          className={`sticky top-0 z-50 transition-all duration-500 ${
            scrolled
              ? "bg-[#0c0618]/97 backdrop-blur-2xl shadow-2xl shadow-violet-950/40"
              : "bg-[#1e0a3c]"
          }`}
          onClick={() => setShowMenu(false)}
        >
          ...
          <div className="max-w-screen-xl mx-auto px-4 sm:px-6 h-14 sm:h-16 flex items-center gap-3 sm:gap-5 border-b border-white/[0.06]">
            ...
            >
              LY<span className="text-pink-400">R</span>A
              <sup
                className="text-white/20 ml-0.5 hidden sm:inline"
                style={{ fontSize: "7px", letterSpacing: "0.4em", verticalAlign: "top", marginTop: "6px" }}
              >
                FASHION
              </sup>
            </button>
            ...
            <div className="flex-1 max-w-2xl relative hidden sm:block">
              <div className="flex items-center bg-white/8 border border-white/10 rounded-xl hover:bg-white/12 focus-within:bg-white focus-within:border-violet-400 transition-all group">
                <svg
                  className="ml-3.5 w-4 h-4 text-white/30 group-focus-within:text-violet-600 flex-shrink-0 transition-colors"
                ...
                {search && (
                  <button
                    onClick={() => setSearch("")}
                    className="mr-3 w-5 h-5 rounded-full bg-white/15 text-white/60 flex items-center justify-center text-xs"
                  >✕</button>
                )}
              </div>
            </div>
            ...
            <div className="flex items-center gap-0.5 ml-auto flex-shrink-0">
              ...
              >
                ...
                {wishlistCount > 0 && (
                  <span
                    className="absolute -top-1 right-0 w-4 h-4 bg-pink-500 rounded-full flex items-center justify-center text-white font-bold shadow"
                    style={{ fontSize: "8px" }}
                  >
                    {wishlistCount}
                  </span>
              ...
              <div className="relative" onClick={(e) => e.stopPropagation()}>
                <button
                  ...
                  className="flex flex-col items-center px-2 sm:px-3 py-1.5 text-white/55 hover:text-white hover:bg-white/8 rounded-xl transition-all"
                >
                  <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.8}>
                    <path strokeLinecap="round" strokeLinejoin="round" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
                  </svg>
                  <span className="hidden sm:block mt-0.5 text-white/35 text-[9px] tracking-wider">
                    {firstName.toUpperCase().slice(0, 8)}
                  </span>
                </button>
                {showMenu && (
                  <div className="absolute right-0 top-full mt-2 w-56 bg-white rounded-2xl shadow-2xl border border-slate-100 overflow-hidden z-50 slide-down">
                    ...
                    <button
                      onClick={handleLogout}
                      className="w-full text-left px-5 py-3 text-sm text-red-500 hover:bg-red-50 transition-colors border-t border-gray-100"
                    >
                      🚪 Sign Out
                    </button>
                  </div>
          ...
          <div className="max-w-screen-xl mx-auto px-4 sm:px-6 py-8 sm:py-12">
            ...
            <div className="lyra-trust-grid mb-10 sm:mb-14">
              {[
                ...
                <div
                  key={o.title}
                  className={`bg-gradient-to-r ${o.from} ${o.to} rounded-2xl p-4 sm:p-5 flex items-center gap-3 sm:gap-4 hover:-translate-y-1 transition-all duration-300 shadow cursor-pointer`}
                >
                  <span className="text-2xl sm:text-3xl">{o.icon}</span>
                  <div>
                    <p className="lyra-serif text-white font-semibold text-sm sm:text-base">{o.title}</p>
                ...
                <div className="flex items-end justify-between mb-5 sm:mb-7">
                  <div>
                    ...
                    <h2 className="lyra-serif text-3xl sm:text-4xl text-gray-900">
                      Shop by <span className="text-violet-600">World</span>
                    </h2>
                  </div>
                  <button className="text-xs sm:text-sm text-violet-600 font-semibold hover:underline">View All →</button>
                </div>
            ...
            <div
              className="rounded-2xl px-4 sm:px-6 py-4 sm:py-5 mb-10 sm:mb-12 flex flex-wrap sm:flex-nowrap items-center gap-3 sm:gap-4 shadow-lg"
              style={{ background: "linear-gradient(135deg,#1e0a3c,#4c1d95,#6d28d9)" }}
            >
              <div className="flex-1 min-w-0">
                <h3 className="lyra-serif text-white text-xl sm:text-2xl">Flash Sale</h3>
              ...
              <span className="bg-pink-500 text-white text-[10px] sm:text-xs font-bold px-2.5 sm:px-3 py-1 rounded-full animate-pulse">
                ⚡ LIVE
              </span>
              <div className="flex items-center gap-1.5 ml-auto sm:ml-0">
                {[[countdown.h,"HRS"],[countdown.m,"MIN"],[countdown.s,"SEC"]].map(([n, l], i) => (
              ...
              <div className="mb-10 sm:mb-14">
                <div className="flex items-end justify-between mb-4 sm:mb-6">
                  <div>
                    <p className="text-[10px] tracking-[0.3em] text-pink-400 uppercase mb-1">✦ Limited Time</p>
                    <h2 className="lyra-serif text-2xl sm:text-3xl text-gray-900">
                      Best <span className="text-pink-600">Deals</span>
                    </h2>
                  </div>
                  <button className="text-xs sm:text-sm text-violet-600 font-semibold hover:underline">View All Deals →</button>
                </div>
            ...
            <section ref={shopRef} id="shop">
                ...
                <div className="flex items-center gap-2 sm:gap-3">
                  {activeBrand && (
                    <button
                      ...
                      className="flex items-center gap-1.5 bg-violet-100 text-violet-700 text-xs font-semibold px-3 sm:px-4 py-1.5 rounded-full hover:bg-violet-200 transition-colors"
                    >
                      {activeBrand} <span className="font-bold">×</span>
                    </button>
                  )}
              ...
              {filtered.length > 0 ? (
                ...
                <div className="text-center py-20 sm:py-24 bg-white rounded-3xl border border-slate-100">
                  ...
                  <p className="lyra-serif text-xl text-gray-700 mb-2">No products found</p>
                  <button
                    onClick={() => { setSearch(""); setActiveBrand(null); setActiveTab("All"); }}
                    className="mt-5 text-sm text-violet-600 font-medium hover:underline"
                  >
                    Clear filters
                  </button>
                </div>
            ...
            <div className="mt-14 sm:mt-20 mb-10">
              <div className="flex items-end justify-between mb-5 sm:mb-7">
                <h2 className="lyra-serif text-2xl sm:text-3xl text-gray-900">
                  Explore <span className="text-violet-600">Collections</span>
                </h2>
              </div>
              <div className="lyra-editorial-grid">
                {(categories || []).slice(0, 4).map((cat, i) => {
                  const overlay = ["#5b21b6","#9d174d","#1f2937","#78350f"][i] || "#1e0a3c";
                  return (
                    <div
                      key={cat._id}
                    ...
                    >
                      <img
                        src={cat.featuredImg || cat.heroImg || SHOWCASE_IMGS[cat.slug] || ""}
                        alt={cat.name}
                      ...
                      <div
                        ...
                        style={{ background: `linear-gradient(to top,${overlay},transparent 65%)` }}
                      />
                      <div className="absolute inset-0 flex flex-col justify-end p-3 sm:p-5">
                        <span className="text-white/50 text-[9px] font-bold uppercase tracking-widest mb-1">Collection</span>
                        <p className="lyra-serif text-white font-light text-base sm:text-xl leading-tight">
                          {cat.label || cat.name}
                        </p>
                        <span className="text-white/45 text-[10px] sm:text-xs mt-1 group-hover:text-white transition-colors">
                          Shop now →
                        </span>
                      </div>
                    </div>
